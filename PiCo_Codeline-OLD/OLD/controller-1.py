...
from config import PROG_MODE_SOURCE, _DEBUG  # bleibt wie gehabt weiter unten
...
class PlayerController:
    ...
    def _load_book(self, uid):
        ...
        if not chapters:
            ...
            self.display.show_status("Kein Album für", uid)
            if _DEBUG == True:
    ...
    def _start_play(self, resume=True):
        ...
        if _DEBUG == True:
            if resume:
                print("[CTRL] resume playback " + str(f))
            else:
                print("[CTRL] start playback " + str(f))
    ...
    def _update_progress_and_feed(self):
        ...
        if _DEBUG == True:
            print("[CTRL] current pos " + str(pos) + "/" + str(total))
        now = utime.ticks_ms()
        if total > 0:
    ...
    def loop(self):
        ...
        if prog:
            ...
            # NFC NUR abfragen, wenn NICHT abgespielt wird
            if not self.playing:
                uid = None
                if utime.ticks_diff(now, self._last_uid_poll) >= 100:
                    self._last_uid_poll = now
                    uid = self.nfc.read_uid()   # hier darf es „schwerer“ sein
                else:
