...
ledRoutes.get('/welcome', (req, res) => {
  if (DEBUG) console.log('GET::'+ledApi+ledUrl+'/welcome');
  ...
  if (acceptsHTML) {
    ...
    res.status(200).render('content', {
      title: 'Welcome to Fairytale LED Controller',
      headline: 'LED Controller',
      subheadline: 'Willkommen',
      messagetext: 'THIS PAGE IS A PLACEHOLDER - COMPONENT WELCOME TO COME LATER'
    });
  } else {
    if (TRACE) console.log("   json request");
    var respEndpoints = {
      response: 'REST API Endpoints available',
      endpoints: ledServiceEndpoints
      };
