...
class SDCard:
    def __init__(self, spi, cs):
        if _DEBUG: print("[SD] initializing SD card class")
        self.spi = spi
        self.cs = cs
    ...
    def init_card(self):
        ...
        if r == _R1_IDLE_STATE:
            if _DEBUG: print("[SD] SD card is V2")
            self.init_card_v2()
        elif r == (_R1_IDLE_STATE | _R1_ILLEGAL_COMMAND):
            if _DEBUG: print("[SD] SD card is V1")
            self.init_card_v1()
        else:
            if _DEBUG: print("[SD] SD card version un known")
        ...
        if _DEBUG: print("[SD] Sectors on CD Card: ", self.sectors)
        
        # CMD16: set block length to 512 bytes
        if self.cmd(16, 512, 0) != 0:
            raise OSError("can't set 512 block size")
        
        if _DEBUG: print("[SD] now initializing SD card for real with speed ", _SD_SPI_RATE)
        self.init_spi(_SD_SPI_RATE)
