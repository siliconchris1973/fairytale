...
class VS1053(BaseAudio):
    ...
    def _wait_dreq(self, timeout_ms=200, loop_sleep_ms=1):
        ...
        while not self.dreq.value():
            if utime.ticks_diff(utime.ticks_ms(), start) > timeout_ms:
                if _DEBUG: print("[VS1053] WARN: DREQ timeout (>{} ms)".format(timeout_ms))
                return False
    ...
    def _write_cmd(self, addr, value):
        ...
        if not self._wait_dreq(200, 1):
            return 0  # oder letzter sinnvoller Wert
    ...
    def _write_data(self, b):
        """Schreibt Audiodaten in kleinen Blöcken zum VS1053.

        Respektiert DREQ: pro DREQ-High werden maximal 32 Byte gesendet,
        um FIFO-Überlauf und daraus resultierende Audio-Artefakte zu vermeiden.
        """
        mv = memoryview(b)
        length = len(mv)
        idx = 0
        ...
        while idx < length:
            # warten, bis der VS1053 mind. 32 Byte Puffer frei hat
            #while not self.dreq.value():
            #    # ein winziges Sleep verhindert komplett "feste" Busyloops
            #    utime.sleep_us(10)
            if not self._wait_dreq(200, 10):
                return
            
            self.xdcs.value(0)
            # pro Burst max. 32 Byte schicken
            n = min(32, length - idx)
    ...
    def _soft_reset(self):
        ...
        # kurze Wartezeit, dann auf DREQ warten
        utime.sleep_ms(2)
        start = utime.ticks_ms()
        while not self.dreq.value():
            if utime.ticks_diff(utime.ticks_ms(), start) > 1000:
                if _DEBUG: print("[VS1053] ERROR: DREQ did not go high within 1s after reset")
                break  # Nicht endlos hängen bleiben
            utime.sleep_ms(1)
    
    # --- Public API ---
    def init(self):
        if _DEBUG: print("[VS1053] init: start")
        ...
        self._soft_reset()
        if _DEBUG: print("[VS1053] init: after reset, DREQ =", self.dreq.value())
        
        # internen Clock-Multiplikator erhöhen
        self._write_cmd(SCI_CLOCKF, 0x9800)  # ~3.0x
        self.set_volume(60)
        if _DEBUG: print("[VS1053] init: basic MODE set")
    
    def set_volume(self, percent: int):
        # VS1053 Volume: 0x00 = max, 0xFEFE = mute. Map 0..100 → 200..0 (grobe Skalierung)
    ...
    def feed(self):
        ...
        """
        if self._paused or not self._f:
            # Wenn pausiert, aber noch Datei offen -> Alive = True
            return False if not self._f else True
        ...
        while chunks_sent < _MAX_FEED_CHUNKS:
            ...
            # Wenn der letzte read kleiner war als _STREAM_CHUNK, ist vermutlich Schluss
