...
/*
   ...
   - iRRemotePin       A3  this is the data input pin of the IR Remote receiver
...
// you may also do a find and replace    if (DEBUG)    with    // if (DEBUG)    to reduce memory impact even a bit more
...
char playTrack(byte, char, char[]);     // plays a track as defined by the global vars: returns -1 for error, 0 for success and track number to play next
...
void setup() {
  ...
  if (playTrack(1, 1, "/system00/welcomef.mp3") == -1) {       // play the welcome sound
    issueWarning("Error playing hello", "", false);
  }
...
static void plrAdjustVolume() {
  ...
  const word sensorValue = analogRead(volumePotPin);
  ...
  if (compareValue > sensorDrift) {
    ...
    const byte soundVolume = sensorValue / 10; // map the sensor value to a sound volume
...
char playAlbum(byte numberOfFiles) {
  ...
  for (byte curTrack = firstTrackToPlay; curTrack <= numberOfFiles; curTrack++) {
    ...
    // holds the next filename to play
    char filename[23];
    // convert the trackNo to a char array
    char curTrackCharNumber[4];
    sprintf(curTrackCharNumber, "%03d", curTrack);
    ...
    playTrack(curTrack, numberOfFiles, filename);
...
static char playTrack(byte trackNo, char numberOfTracks, char filename[]) {
  ...
  while (musicPlayer.playingMusic || musicPlayer.paused()) {
    ...
    if (((millis()-lightStartUpTime) > maxLightTime) && lightOn) { 
      if (DEBUG) Serial.println(F("turn light off")); 
      switchLightState(); 
    }
