...
const word btnLight     = 1021;     // The value we receive from analog input if the Light On/Off Button is pressed
const word btnPause     = 933;      // The value we receive from analog input if the Pause Button is pressed
const word btnNext      = 1002;     // The value we receive from analog input if the Next (aka Fast Forward) Button is pressed
const word btnPrev      = 991;      // The value we receive from analog input if the Previos (aka Prev or Rewind) Button is pressed
...
char playAlbum(byte numberOfFiles) {
  ...
  for (byte curTrack = firstTrackToPlay; curTrack <= numberOfFiles; curTrack++) {
    ...
    if (nextTrackToPlay == -1) {
      loopWarningMessageFileNotFound = false; // set loop warning message so we don't show this warning in the next loop
      issueWarning(5, "playback failed", true);
      break;
